...
async def line_webhook(request: Request, db: Session = Depends(database.get_db)):
    ...
    for evt in body_json.get('events', []):
        ...
        if evt['message']['type'] == 'image':
            ...
            get_image_from_LINE(
                message_id, body_json['destination'], evt['replyToken'], db)
        elif evt['message']['type'] == 'text':
            message_id = evt['message']['id']
            message = evt['message']['text']
            if message.isdigit():
                get_response_from_text(
                    body_json['destination'], message, evt['replyToken'], db)
            else:
...
def upload_image_to_blob(message_id: str, image_data, user_id: str, replyToken: str, db: Session):
    try:
        ...
        # Overwrite if blob already exists
        respons = blob_client.upload_blob(image_data, overwrite=True)
        if respons:
            get_response_from_image(BLOB_URL, user_id, replyToken, db)
        print(
            f"Image uploaded as '{message_id}' in container '{CONTAINER_NAME}'.")
...
def get_response_from_image(IMAGE_PATH, user_id, replyToken, db: Session):
...
def get_response_from_text(user_id, message, replyToken, db):
    ...
    if retrieved_state.stage_number == 3:
        ...
        send_message("Please send me chat screenshot first.", replyToken)
